...
| 36 | 웨이브 | 페이즈 장기화 — WaveData에 `phaseDuration` 필드 추가, WaveManager.SpawnWave()를 순차 블록 스폰에서 entry별 라운드로빈(인터리브) 스폰으로 재작성해 여러 종류가 섞여서 오래 나오도록 변경. Wave1~5.asset의 phaseDuration/count 값 상향 | 확장(선택) | 재작성 (WaveManager.cs, WaveData.cs) |
