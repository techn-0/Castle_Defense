...
10. #10 원거리: 사거리 내 벽 감지 시 정지·공격 (이후 수정 — 아래 참고)
    ...
    - 버그 수정: 이동 사격으로 바꾼 뒤 "공격이 끝나고 다시 이동할 때 Idle 모션으로 그냥 전진한다"는 문제 발견. 원인은 `SPUM_Prefabs.PlayAnimation(ATTACK, ...)`이 애니메이터의 `1_Move` bool을 내부적으로 항상 `false`로 꺼버리는데, `Enemy.SetSpumMoving()`은 캐시된 `spumMoving` 플래그가 이미 `true`(계속 이동 중이었으므로)라 값이 안 바뀌었다고 보고 `PlayAnimation(MOVE)`를 다시 호출하지 않아 `1_Move`가 true로 복구되지 않았던 것. 원거리 공격 트리거 직후 `spumMoving = false`로 캐시를 무효화해, 같은 프레임에 이어지는 이동 로직의 `SetSpumMoving(true)` 호출이 다시 걸리도록(=Move bool 복구) 수정.
    - 요청 추가: 원거리 공격 시 투사체가 눈에 보이길 원해서 `EnemyProjectile.cs`(신규) 추가 — 데미지는 `Enemy.cs`가 공격 판정 시점에 이미 즉시 적용하므로(원거리 이동사격 특성상 명중 판정을 투사체 도달 시점까지 늦추면 로직이 복잡해짐), 이 투사체는 목표 지점까지 날아가는 모습만 보여주는 순수 시각효과(충돌 판정 없음)로 구현. `Enemy.cs`에 `projectilePrefab`/`projectileSpeed` 필드와 `SpawnProjectile()` 헬퍼 추가, 원거리 공격 판정 시 호출. Player의 `Projectile.cs`/`Projectile.prefab`과 동일한 패턴이나 물리 충돌이 없다는 점만 다름 — 원거리 적 프리팹(Enemy2 등)의 `Projectile Prefab` 필드에 비주얼용 프리팹(예: 화살 스프라이트 + `EnemyProjectile` 컴포넌트, 콜라이더 불필요) 연결 필요(씬 배선 대기).
    - 버그 수정: 위 필드에 사용자가 새 프리팹 대신 이미 있던 Player용 `Projectile.prefab`을 그대로 연결해서, 화살이 아예 안 보이는 문제 발생. 원인 분석 — `Projectile.prefab`엔 `Projectile` 컴포넌트(물리 충돌 판정용, Collider2D+Rigidbody2D 포함)가 붙어 있는데, `SpawnProjectile()`은 `EnemyProjectile` 컴포넌트를 찾아 `Init()`을 호출하는 구조라 이 프리팹엔 `EnemyProjectile`이 없어 아무 데도 초기화가 안 됨 → 게다가 스폰 위치가 쏘는 적 자신의 위치와 겹치기 때문에, 다음 물리 스텝에서 `Projectile`의 트리거가 자기 자신의 콜라이더와 곧바로 충돌 판정돼 `TakeDamage(0)` 호출 후 즉시 파괴되어(파괴 시점이 매우 빨라 화면에 한 프레임도 안 보임) 눈에 보이지 않았던 것. 별도 프리팹을 새로 만들 필요 없이 `Enemy.SpawnProjectile()`에서 인스턴스화 직후 기존 `Projectile` 컴포넌트와 `Collider2D`를 `Destroy()`로 제거하고 `EnemyProjectile`을 `AddComponent`로 붙여 초기화하도록 수정 — 어떤 프리팹을 연결하든(전용 프리팹이든 Player와 공유하는 프리팹이든) 항상 순수 시각효과로만 동작하도록 방어.
